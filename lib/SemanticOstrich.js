...
export class SemanticOstrich {
  ...
  init(pathDataset, pathLanguage, readonly) {
    return new Promise((resolve, reject) => {
      ostrich.fromPath(pathDataset, !!readonly, async (error, store) => {
        if (error) {
          return reject(error);
        }
        this._promisifyStore(store);
        this._storeDataset = store;
        if (pathLanguage) {
          ostrich.fromPath(pathLanguage, !!readonly, async (error, store) => {
            if (error) {
              return reject(error);
            }
            this._promisifyStore(store);
            this._storeLanguage = store;
            resolve();
          });
        } else {
          resolve();
  ...
  async semanticSearchTriplesVersionMaterialized(rules, s, p, o, optionsDataset, optionsLanguage) {
    ...
    let triples = await this.queryStores('searchTriplesVersionMaterialized', s, p, o, optionsDataset, optionsLanguage);
    ...
    while (reasonTriples.length > 0) {
      const inferredTriples = await this.inferTriples(reasonTriples, appliedRules,
        (s, p, o) => this.queryStores('searchTriplesVersionMaterialized', s, p, o, optionsDataset, optionsLanguage));
  ...
  async semanticSearchTriplesDeltaMaterialized(rules, s, p, o, optionsDataset, optionsLanguage) {
    ...
    // Get results from original pattern
    let triples = await this.queryStores('searchTriplesDeltaMaterialized', s, p, o, optionsDataset, optionsLanguage);
  ...
  async semanticSearchTriplesVersion(rules, s, p, o, optionsDataset, optionsLanguage) {
    ...
    // Get results from original pattern
    let triples = await this.queryStores('searchTriplesVersion', s, p, o, optionsDataset, optionsLanguage);
